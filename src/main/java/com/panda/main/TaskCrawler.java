...
import com.panda.service.ActiveJmxService;
...
public class TaskCrawler {
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            List<TaskDO> taskList = TaskDao.getInstance().selectListByStatus(TaskStatusEnum.NOT_RUNNING.getValue());
            ...
            if (queueSize == null || queueSize < QUEUE_MAX_NUM) {
                ...
                for (int i = 0; i < taskList.size(); i++) {
                    TaskDO nowTask = taskList.get(i);
                    String message = new Gson().toJson(nowTask);
                    try {
                        service.sendMessage(queueName, message);

                        // 修改任务状态为 正在运行中
                        nowTask.setTaskStatus(TaskStatusEnum.RUNNING.getValue());
                        TaskDao.getInstance().updateStatus(nowTask);
                    } catch (Exception e) {
